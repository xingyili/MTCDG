...
from sklearn.metrics import auc as js_auc
...
def calculate_metrics(y_true, y_pred, y_score):
    ...
    auprc = js_auc(recall, precision)
    f1 = f1_score(y_true, y_pred)
    auc = roc_auc_score(y_true, y_score)
    return acc, auprc, f1, auc
...
def resplit(data, cv, device):
    skf = StratifiedKFold(n_splits=cv, random_state=42, shuffle=True)
    union_mask = data.train_mask | data.val_mask | data.test_mask
    true_indices = torch.nonzero(union_mask, as_tuple=False).squeeze()
    y = data.y.squeeze()[union_mask.squeeze()]
    aaa = y.cpu().numpy()
    y = y.to(device)
    ...
    val_cv = []
    test_cv = []
    for train_index, test_index in skf.split(true_indices.cpu().numpy(), y.cpu().numpy()):  # get index
        tr_index = true_indices[train_index]
        te_index = true_indices[test_index]
        aaa = y.cpu().numpy()
        tr_index, val_index, _, _ = train_test_split(tr_index, aaa[train_index], test_size=0.25, shuffle=True,
                                                     stratify=aaa[train_index])
        train_cv.append(tr_index)
        val_cv.append(val_index)
        test_cv.append(te_index)

    train_mask = []
    val_mask = []
    test_mask = []
    for train in train_cv:
        mask = torch.tensor([False] * data.x.shape[0], dtype=torch.bool)
        mask[train] = True
...
def loss_to_log(file, loss_log, best_epoch):
    print(file)
...
def paint_roc(plt, y_true, y_score, i):
    fpr, tpr, _ = roc_curve(y_true, y_score)
    roc_auc = js_auc(fpr, tpr)
    plt.plot(fpr, tpr, label='AUC in fold %d is %0.2f' % (i, roc_auc))


def paint_pr(plt, y_true, y_score, i):
    precision, recall, _ = precision_recall_curve(y_true, y_score)
    auprc = js_auc(recall, precision)  # the value of roc_auc1
    plt.plot(recall, precision, label='AUPRC in fold %d is %0.2f' % (i, auprc))


def show_auc(y_true, y_score, cv):
    plt.figure()
    ...
    for i in range(cv):
        paint_roc(plt, y_true[i], y_score[i], i)

    plt.legend()
    plt.savefig('./visual/AUC.png')
    plt.show()


def show_auprc(y_true, y_score, cv):
    plt.figure()
    plt.plot([1, 0], linestyle="--", color='black')
    plt.xlim([0, 1])  # the range of x-axis
    plt.ylim([0, 1])  # the range of y-axis
    ...
    for i in range(cv):
        paint_pr(plt, y_true[i], y_score[i], i)

    plt.legend()
    plt.savefig('./visual/AUPRC.png')
    plt.show()
